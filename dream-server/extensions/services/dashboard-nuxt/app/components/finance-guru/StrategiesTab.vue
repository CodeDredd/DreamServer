<!--
  Finance-Guru Strategies Tab (refactor, Phase 4 Welle C.2). Pendant zu
  ...
  Aufbau:
    ...
    3. Strategy-Liste (links) + Strategy-Detail (rechts, UTable fuer
       Positions + Trade-Liste).
    4. Cycle-Log (UTable mit Filtern + UModal-Detail).
    5. Enrichment-Workflow-Log (n8n-Anreicherung).
-->
...
import EnrichmentRunsTable from '~/components/finance-guru/EnrichmentRunsTable.vue'
import {
  formatEur,
  formatPct,
  pnlToneClass,
...
<template>
  <div class="space-y-6 p-4">
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        <h2 class="flex items-center gap-2 text-xl font-bold text-default">
          <UIcon name="i-lucide-trending-up" class="size-5 text-primary" />
          Paper-Trade Strategien
        </h2>
        <p class="mt-0.5 text-xs text-muted">
          €1000 Seed pro Strategie · Ziel 10 %/Woche · {{ fg.schedule.value?.cron || 'kein Cron' }}
          ({{ fg.schedule.value?.tz || 'UTC' }})
          <span v-if="fg.nextRun.value">· next {{ relTime(fg.nextRun.value) }}</span>
        </p>
      </div>
      <div class="flex items-center gap-2">
        <UButton color="neutral" variant="ghost" icon="i-lucide-refresh-cw" square
                 title="Refresh" @click="fg.fetchAll()"
        />
    ...
    />
    <div v-if="fg.loading.value && !fg.strategies.value.length" class="space-y-4">
      <USkeleton class="h-8 w-1/3" />
      <USkeleton class="h-32 rounded-xl" />
      <USkeleton class="h-64 rounded-xl" />
    </div>
    <div v-if="fg.aggregate.value" class="grid grid-cols-2 gap-3 md:grid-cols-5">
      <UCard>
        <div class="flex items-center gap-2 text-xs uppercase tracking-wider text-muted">
          <UIcon name="i-lucide-wallet" class="size-3.5" /> Seeded
        </div>
      ...
      <UCard>
        <div class="flex items-center gap-2 text-xs uppercase tracking-wider text-muted">
          <UIcon name="i-lucide-coins" class="size-3.5" /> Equity
        </div>
      ...
      <UCard>
        <div class="flex items-center gap-2 text-xs uppercase tracking-wider text-muted">
          <UIcon name="i-lucide-target" class="size-3.5" /> Vs Ziel 10 %/Wo
        </div>
        <div class="text-lg font-semibold"
      ...
      <UCard>
        <div class="flex items-center gap-2 text-xs uppercase tracking-wider text-muted">
          <UIcon name="i-lucide-activity" class="size-3.5" /> Realised
        </div>
        <div class="text-lg font-semibold"
      ...
      <UCard>
        <div class="flex items-center gap-2 text-xs uppercase tracking-wider text-muted">
          <UIcon name="i-lucide-history" class="size-3.5" /> Cycles 24h
        </div>
        <div class="text-lg font-semibold text-default">
        ...
        >
          {{ fg.cycleSummary.value?.errors ?? 0 }} Errors gesamt
        </div>
      </UCard>
    </div>
      ...
      <div class="space-y-2 lg:col-span-1">
        ...
        >
          ...
          <div class="flex items-center gap-2 text-[11px] text-muted">
            ...
            <span>· {{ fg.ledgers.value[s.name]?.kpi?.n_trades ?? 0 }} tr</span>
          ...
          <UCard>
            <template #header>
              <div class="flex items-start justify-between gap-4">
                <div>
                  <h3 class="font-mono text-base font-semibold text-default">
          ...
          <UCard :ui="{ body: 'p-0' }">
            <template #header>
              <div class="flex items-center gap-2">
                ...
                <h4 class="text-sm font-semibold">
                  Trades ({{ selectedLedger.trades?.length ?? 0 }})
                </h4>
